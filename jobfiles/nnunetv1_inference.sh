#!/bin/bash
#SBATCH --job-name=nnv1_inf    # job name
#SBATCH --nodes=1              # number of nodes to use
#SBATCH --ntasks=1             # total number of tasks across all nodes
#SBATCH --cpus-per-task=10     # cpu-cores per task (>1 if multi-threaded tasks)
#SBATCH --mem=64gb             # total memory (--mem-per-cpu per cpu-core 4G is default)
#SBATCH --time=1-23:59:00      # total run time limit (HH:MM:SS)
#SBATCH --gres=gpu:1           # number of gpus per node
#SBATCH --output=/home/jaime.barrancohernandez/results/nnUNet_predict.%N.%j.%a.out  # output log
#SBATCH --error=/home/jaime.barrancohernandez/results/nnUNet_predict.%N.%j.%a.err   # error log
#SBATCH --mail-type=BEGIN,END  # send email when job begins and ends
#SBATCH --mail-user=$MAIL_USER

apptainer exec \
    --nv \
    --bind /home/jaime.barrancohernandez/shared_datasets/nnunetv1/nnUNet:/opt/nnunet_resources \
    --bind /home/jaime.barrancohernandez/shared_datasets/nnunetv1/nnUNet/nnUNet_inference/input:/input \
    --bind /home/jaime.barrancohernandez/results/nnunetv1:/output \
    /home/jaime.barrancohernandez/shared_datasets/nnunetv1/nnunetv1.sif \
    nnUNet_predict \
    -i /input \
    -o /output \
    -tr nnUNetTrainerV2 \
    -ctr nnUNetTrainerV2CascadeFullRes \
    -m 3d_fullres \
    -p nnUNetPlansv2.1 \
    -t Task313_Eye