...
def getSegmentation(user_email: str, paths: UserPaths, ongoing_job_id: Callable[[str], None] | None = None) -> str:
    ...
    process = subprocess.Popen(inference_command,
                                shell=True,
    ...
    try:
        ...
        if process.returncode != 0:
            raise subprocess.CalledProcessError(process.returncode, inference_command, stderr=sub_sdterr)
    except Exception:
        if job_id:
